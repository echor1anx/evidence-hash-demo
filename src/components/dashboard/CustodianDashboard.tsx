...
import { ArrowRight, FileText, Search, Clock, ShieldCheck, Box } from "lucide-react";

export default function CustodianDashboard({ user, cases }: { user: User; cases: ICase[] }) {
    const [searchQuery, setSearchQuery] = useState("");
    const [filterType, setFilterType] = useState<"all" | "secured" | "active">("all");
                ...
                <div className="space-y-6 lg:transform lg:-translate-y-12">
                    <div>
                        ...
                        <p className="text-slate-400 mt-4 text-lg font-medium flex items-center gap-2">
                            Custodian, <span className="text-white">{user.name}</span>
                        </p>
                    </div>

                    <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-6 py-4 rounded-2xl flex items-start gap-4">
                        <ShieldCheck size={24} className="shrink-0 mt-0.5" />
                        <p className="text-sm font-medium leading-relaxed">
                            You are assigned to store, manage, and provide access to cryptographic evidence. Original evidence modification and new ledger initialization are restricted.
                        </p>
                    </div>
                    ...
                    <div className="mt-8 grid grid-cols-2 gap-4">
                        <button 
                            ...
                            className={`p-5 rounded-2xl backdrop-blur-xl text-left transition-all outline-none focus:ring-2 focus:ring-emerald-500/50 ${
                                filterType === "secured" 
                                    ? "bg-emerald-500/20 border border-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.3)] transform scale-[1.02]" 
                                    : "bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20"
                            }`}
                        >
                            <div className="text-emerald-400 mb-2"><Box size={20} /></div>
                            <p className="text-2xl font-bold text-white leading-none mt-1">{totalEvidenceCount}</p>
                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-2">Secured Artifacts</p>
                        </button>
                        <button 
                            onClick={() => setFilterType(filterType === "active" ? "all" : "active")}
                            className={`p-5 rounded-2xl backdrop-blur-xl text-left transition-all outline-none focus:ring-2 focus:ring-teal-500/50 ${
                                filterType === "active" 
                                    ? "bg-teal-500/20 border border-teal-400 shadow-[0_0_15px_rgba(45,212,191,0.3)] transform scale-[1.02]" 
                                    : "bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20"
                            }`}
                        >
                            <div className="text-teal-400 mb-2"><FileText size={20} /></div>
                            <p className="text-2xl font-bold text-white leading-none mt-1">{securedCasesCount}</p>
            ...
            <div className="lg:col-span-8">
                <div className="bg-white rounded-[2rem] shadow-2xl p-6 md:p-10 min-h-[600px] border border-slate-100 flex flex-col">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                        <h2 className="text-2xl font-bold text-slate-800">Custodial Ledgers</h2>
                    ...
                    {filteredCases.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center text-center py-20 px-4">
                            ...
                            <p className="text-slate-500 max-w-sm">
                                {searchQuery ? "No ledgers match your search query." : "No case ledgers enter the system yet."}
                            </p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 gap-4">
                            {filteredCases.map((c) => (
                                <Link href={`/dashboard/cases/${c._id as unknown as string}`} key={c._id as unknown as string}>
                                    <div className="group bg-white border border-slate-200 p-6 rounded-2xl hover:border-emerald-300 hover:shadow-xl transition-all duration-300 relative overflow-hidden flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mb-3 mt-1">
                                                <div className="flex items-center gap-1 text-xs font-semibold bg-slate-50 text-slate-500 px-2 py-1 rounded border border-slate-100">
                                                    <Box size={12} />
